...
function saveInputState() {
    const state = {};
    const ids = [
        'itemQuantityInput', 'packSizeInput', 'rarityInput', 'rareMonsterInput', 'magicMonsterInput', 'waystoneInput', 'deliriumInput',
        'packAdditionCheckbox', 'corruptedCheckbox', 'nonCorruptedCheckbox',
        'beastBulkThreshold', 'beastBulkThresholdMax', 'scarabBulkThreshold', 'scarabBulkThresholdMax',
        'tattooBulkThreshold', 'tattooBulkThresholdMax', 'runegraftBulkThreshold', 'runegraftBulkThresholdMax'
    ];
    ids.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            // キー名を統一 (Input や Checkbox を除去)
...
function loadInputState() {
    ...
    const mapping = {
        ...
        'scarabBulkThresholdMax': state.scarabBulkThresholdMax || '',
        'tattooBulkThreshold': state.tattooBulkThreshold || '10',
        'tattooBulkThresholdMax': state.tattooBulkThresholdMax || '',
        'runegraftBulkThreshold': state.runegraftBulkThreshold || '10',
        'runegraftBulkThresholdMax': state.runegraftBulkThresholdMax || ''
    };
...
function saveBeastCheckboxState() {
  ...
  localStorage.setItem('poe2_beastCheckboxState', JSON.stringify(state));
}

// チェックボックス状態復元
function loadBeastCheckboxState() {
  const saved = localStorage.getItem('poe2_beastCheckboxState');
  if (saved) {
...
function saveBeastProfile() {
  ...
  localStorage.setItem('poe2_beastProfiles', JSON.stringify(beastProfiles));
...
function saveScarabCheckboxState() {
  ...
  localStorage.setItem('poe2_scarabCheckboxState', JSON.stringify(state));
}

function loadScarabCheckboxState() {
  const saved = localStorage.getItem('poe2_scarabCheckboxState');
  if (saved) {
    try {
...
function saveScarabProfile() {
  ...
  localStorage.setItem('poe2_scarabProfiles', JSON.stringify(scarabProfiles));
...
function saveTattooCheckboxState() {
  ...
  localStorage.setItem('poe2_tattooCheckboxState', JSON.stringify(state));
}

function loadTattooCheckboxState() {
  const saved = localStorage.getItem('poe2_tattooCheckboxState');
  if (saved) {
    try {
...
function saveTattooProfile() {
  ...
  localStorage.setItem('poe2_tattooProfiles', JSON.stringify(tattooProfiles));
...
function saveRunegraftCheckboxState() {
  ...
  localStorage.setItem('poe2_runegraftCheckboxState', JSON.stringify(state));
}

function loadRunegraftCheckboxState() {
  const saved = localStorage.getItem('poe2_runegraftCheckboxState');
  if (saved) {
    try {
...
function saveRunegraftProfile() {
  ...
  localStorage.setItem('poe2_runegraftProfiles', JSON.stringify(runegraftProfiles));
...
//     updateRunegraftRegex();
    ...
    // 各カテゴリのプロファイルを読み込み
    const savedScarabProfiles = localStorage.getItem('poe2_scarabProfiles');
    if (savedScarabProfiles) scarabProfiles = JSON.parse(savedScarabProfiles) || {};
//     updateScarabProfileList();

    const savedTattooProfiles = localStorage.getItem('poe2_tattooProfiles');
    if (savedTattooProfiles) tattooProfiles = JSON.parse(savedTattooProfiles) || {};
//     updateTattooProfileList();

    const savedRunegraftProfiles = localStorage.getItem('poe2_runegraftProfiles');
