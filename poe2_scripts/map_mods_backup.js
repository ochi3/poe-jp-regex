// Auto-generated by tools_poe2/extract_map_mods.py
const mapModList = {
  "レアモンスターは追加のモッドを#個持つ": {
    "mod": "レアモンスターは追加のモッドを#個持つ",
    "engMod": "Rare Monsters have # additional Modifier|#% increased number of Rare Monsters",
    "value": "1|10|25",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2550456553",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 25,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアにはアンデッドが追加で(##)パック出現する": {
    "mod": "エリアにはアンデッドが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Undead",
    "value": "12-13|10|10",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_240445958"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 10,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアには獣が追加で(##)パック出現する": {
    "mod": "エリアには獣が追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Beasts",
    "value": "12-13|10|5",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3757259819"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 5,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアにはエゾマイトモンスターが追加で(##)パック出現する": {
    "mod": "エリアにはエゾマイトモンスターが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Ezomyte Monsters",
    "value": "12-13|10|5",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1436812886"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 5,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアにはファリドゥンモンスターが追加で(##)パック出現する": {
    "mod": "エリアにはファリドゥンモンスターが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Faridun Monsters",
    "value": "12-13|10|10",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_4130878258"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 10,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアにはヴァールモンスターが追加で(##)パック出現する": {
    "mod": "エリアにはヴァールモンスターが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Vaal Monsters|#% increased number of Magic Monsters",
    "value": "12-13|10|18",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_4181857719",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 18
  },
  "エリアには鉄の護衛が追加で(##)パック出現する": {
    "mod": "エリアには鉄の護衛が追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Iron Guards|#% increased number of Rare Monsters",
    "value": "12-13|10|13",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2949706590",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 13,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアには感染したモンスターが追加で(##)パック出現する": {
    "mod": "エリアには感染したモンスターが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Plagued Monsters",
    "value": "12-13|10|10",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3592067990"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 10,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアには昇華したモンスターが追加で(##)パック出現する": {
    "mod": "エリアには昇華したモンスターが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Transcended Monsters|#% increased number of Rare Monsters",
    "value": "12-13|10|13",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1689473577",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 13,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアには茨のモンスターが追加で(##)パック出現する": {
    "mod": "エリアには茨のモンスターが追加で(##)パック出現する",
    "engMod": "Area contains (##) additional packs of Bramble Monsters",
    "value": "12-13|10|5",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3309089125"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 5,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはダメージの(##)%を追加火ダメージとして与える": {
    "mod": "モンスターはダメージの(##)%を追加火ダメージとして与える",
    "engMod": "Monsters deal (##)% of Damage as Extra Fire|#% increased number of Magic Monsters",
    "value": "26-30|15|28",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_92381065",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 28
  },
  "モンスターはダメージの(##)%を追加冷気ダメージとして与える": {
    "mod": "モンスターはダメージの(##)%を追加冷気ダメージとして与える",
    "engMod": "Monsters deal (##)% of Damage as Extra Cold",
    "value": "26-30|15|22",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_211727"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 22,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはダメージの(##)%を追加雷ダメージとして与える": {
    "mod": "モンスターはダメージの(##)%を追加雷ダメージとして与える",
    "engMod": "Monsters deal (##)% of Damage as Extra Lightning",
    "value": "26-30|15|10",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_512071314"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 10,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターのダメージが(##)%増加する": {
    "mod": "モンスターのダメージが(##)%増加する",
    "engMod": "(##)% increased Monster Damage|#% increased number of Rare Monsters",
    "value": "26-30|20|28",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1890519597",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 20,
    "map_number_of_rare_packs_+%": 28,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターのアタックスピード、キャストスピードおよび移動スピードが(##)%増加する": {
    "mod": "モンスターのアタックスピード、キャストスピードおよび移動スピードが(##)%増加する",
    "engMod": "Monsters have (##)% increased Attack, Cast and Movement Speed",
    "value": "10-15|0|0|25|12",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3909654181"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 12,
    "map_map_item_drop_chance_+%": 25,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターのクリティカルヒット率が(##)%増加する|モンスターのクリティカルダメージボーナス +(##)%": {
    "mod": "モンスターのクリティカルヒット率が(##)%増加する|モンスターのクリティカルダメージボーナス +(##)%",
    "engMod": "Monsters have (##)% increased Critical Hit Chance|+(##)% to Monster Critical Damage Bonus",
    "value": "260-300|26-30|15|10",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2753083623",
      "explicit.stat_57326096"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 10,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターのライフが(##)%上昇する": {
    "mod": "モンスターのライフが(##)%上昇する",
    "engMod": "(##)% more Monster Life|#% increased number of Rare Monsters",
    "value": "40-49|15|26",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_95249895",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 26,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターの元素耐性 +(##)%": {
    "mod": "モンスターの元素耐性 +(##)%",
    "engMod": "+(##)% Monster Elemental Resistances",
    "value": "35-40|10|18",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1054098949"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 18,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはアーマーが高い": {
    "mod": "モンスターはアーマーが高い",
    "engMod": "Monsters are Armoured|#% increased number of Rare Monsters",
    "value": "50|15|22",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2539290279",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 22,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターは回避力が高い": {
    "mod": "モンスターは回避力が高い",
    "engMod": "Monsters are Evasive",
    "value": "50|15|9",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2570249991"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 9,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターは最大ライフの#%を追加最大エナジーシールドとして獲得する": {
    "mod": "モンスターは最大ライフの#%を追加最大エナジーシールドとして獲得する",
    "engMod": "Monsters gain #% of maximum Life as Extra maximum Energy Shield",
    "value": "20|15|17",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2887760183"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 17,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはヒット時に(##)%の確率で毒を付与する": {
    "mod": "モンスターはヒット時に(##)%の確率で毒を付与する",
    "engMod": "Monsters have (##)% chance to Poison on Hit",
    "value": "30-40|10|7",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_95221307"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 7,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはヒット時に(##)%の確率で出血を付与する": {
    "mod": "モンスターはヒット時に(##)%の確率で出血を付与する",
    "engMod": "Monsters have (##)% chance to inflict Bleeding on Hit|#% increased number of Magic Monsters",
    "value": "15-20|10|27",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2506820610",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 27
  },
  "モンスターのスタン閾値が(##)%増加する|モンスターは状態異常閾値が(##)%増加する": {
    "mod": "モンスターのスタン閾値が(##)%増加する|モンスターは状態異常閾値が(##)%増加する",
    "engMod": "Monsters have (##)% increased Stun Threshold|Monsters have (##)% increased Ailment Threshold|#% increased number of Magic Monsters",
    "value": "70-79|70-79|10|30",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_4101943684",
      "explicit.stat_1994551050",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 30
  },
  "モンスターは与えた物理ダメージの(##)%と同量のアーマーを破壊する": {
    "mod": "モンスターは与えた物理ダメージの(##)%と同量のアーマーを破壊する",
    "engMod": "Monsters Break Armour equal to (##)% of Physical Damage dealt|#% increased number of Rare Monsters",
    "value": "30-45|10|21",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1879340377",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 21,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターの命中力が(##)%増加する": {
    "mod": "モンスターの命中力が(##)%増加する",
    "engMod": "Monsters have (##)% increased Accuracy Rating",
    "value": "35-45|10|7",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1588049749"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 7,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはダメージの(##)%を追加混沌ダメージとして与える": {
    "mod": "モンスターはダメージの(##)%を追加混沌ダメージとして与える",
    "engMod": "Monsters deal (##)% of Damage as Extra Chaos",
    "value": "26-30|15|23",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2200661314"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 23,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターのスタン蓄積が#%増加する": {
    "mod": "モンスターのスタン蓄積が#%増加する",
    "engMod": "Monsters have #% increased Stun Buildup|#% increased number of Magic Monsters",
    "value": "125|15|30",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_115425161",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 30
  },
  "モンスターが付与する可燃性の強度が#%増加する|モンスターの凍結蓄積が#%増加する|モンスターの感電付与確率が#%増加する": {
    "mod": "モンスターが付与する可燃性の強度が#%増加する|モンスターの凍結蓄積が#%増加する|モンスターの感電付与確率が#%増加する",
    "engMod": "Monsters inflict #% increased Flammability Magnitude|Monsters have #% increased Freeze Buildup|Monsters have #% increased Shock Chance",
    "value": "125|125|125|15|18",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2508044078",
      "explicit.stat_3998863698",
      "explicit.stat_1984618452"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 18,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターは投射物を追加で#個放つ": {
    "mod": "モンスターは投射物を追加で#個放つ",
    "engMod": "Monsters fire # additional Projectiles",
    "value": "2|25|14",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1309819744"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 14,
    "map_map_item_drop_chance_+%": 25,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアはエンフィーブルで呪われている": {
    "mod": "エリアはエンフィーブルで呪われている",
    "engMod": "Players are periodically Cursed with Enfeeble|#% increased number of Magic Monsters",
    "value": "1|20|36",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2029171424",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 20,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 36
  },
  "エリアはテンポラルチェーンで呪われている": {
    "mod": "エリアはテンポラルチェーンで呪われている",
    "engMod": "Players are periodically Cursed with Temporal Chains",
    "value": "1|20|11",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1629357380"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 11,
    "map_map_item_drop_chance_+%": 20,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアはエレメンタルウィークネスで呪われている": {
    "mod": "エリアはエレメンタルウィークネスで呪われている",
    "engMod": "Players are periodically Cursed with Elemental Weakness",
    "value": "1|20|23",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_554690751"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 23,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 20,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアには発火領域がまだらに存在する": {
    "mod": "エリアには発火領域がまだらに存在する",
    "engMod": "Area has patches of Ignited Ground|#% increased number of Magic Monsters",
    "value": "6000|6|20|15|33|1033",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_133340941",
      "explicit.stat_3873704640"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 33
  },
  "エリアには冷却領域がまだらに存在する": {
    "mod": "エリアには冷却領域がまだらに存在する",
    "engMod": "Area has patches of Chilled Ground",
    "value": "30|5|26|15|20|1014",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_349586058"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 20,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "エリアには感電領域がまだらに存在する": {
    "mod": "エリアには感電領域がまだらに存在する",
    "engMod": "Area has patches of Shocked Ground",
    "value": "1|7|17|15|10|1011",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3477720557"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 10,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターのダメージは(##)%の元素耐性を貫通する": {
    "mod": "モンスターのダメージは(##)%の元素耐性を貫通する",
    "engMod": "Monster Damage Penetrates (##)% Elemental Resistances",
    "value": "13-15|20|25",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_1898978455"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 25,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 20,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "プレイヤーの耐性の最大値が-(##)%": {
    "mod": "プレイヤーの耐性の最大値が-(##)%",
    "engMod": "-(##)% maximum Player Resistances",
    "value": "-12--10|25|13",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3376488707"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 13,
    "map_map_item_drop_chance_+%": 25,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "全てのプレイヤーの獲得フラスコチャージが(##)%減少する": {
    "mod": "全てのプレイヤーの獲得フラスコチャージが(##)%減少する",
    "engMod": "Players gain (##)% reduced Flask Charges",
    "value": "30-35|-15|-9",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_2549889921"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 9,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "全てのプレイヤーのライフとエナジーシールドの回復レートが(##)%低下する": {
    "mod": "全てのプレイヤーのライフとエナジーシールドの回復レートが(##)%低下する",
    "engMod": "Players have (##)% less Recovery Rate of Life and Energy Shield",
    "value": "36-40|-20|-22",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_4181072906"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 22,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 20,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "全てのプレイヤーのクールダウン解消レートが(##)%低下する": {
    "mod": "全てのプレイヤーのクールダウン解消レートが(##)%低下する",
    "engMod": "Players have (##)% less Cooldown Recovery Rate",
    "value": "25-30|-15|-19",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      ""
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 19,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 15,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターがクリティカルヒットから受ける追加ダメージが(##)%減少する": {
    "mod": "モンスターがクリティカルヒットから受ける追加ダメージが(##)%減少する",
    "engMod": "Monsters take (##)% reduced Extra Damage from Critical Hits|#% increased number of Rare Monsters",
    "value": "-30--26|-10|-24",
    "type": "Suffix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_337935900",
      "explicit.stat_3793155082"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 24,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターに対する呪いの効果が(##)%低下する": {
    "mod": "モンスターに対する呪いの効果が(##)%低下する",
    "engMod": "(##)% less effect of Curses on Monsters",
    "value": "40-50|-10|-16",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3796523155"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 16,
    "map_pack_size_+%": 0,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  },
  "モンスターはヒット時に(##)%の確率でパワーチャージ、フレンジーチャージおよびエンデュランスチャージのスタックを盗む": {
    "mod": "モンスターはヒット時に(##)%の確率でパワーチャージ、フレンジーチャージおよびエンデュランスチャージのスタックを盗む",
    "engMod": "Monsters have (##)% chance to steal Power, Frenzy and Endurance charges on Hit",
    "value": "20-25|10|7",
    "type": "Prefix",
    "weight": 1,
    "tier": 0,
    "engRegex": "",
    "Regex": "",
    "tradeStatIds": [
      "explicit.stat_3222482040"
    ],
    "map_item_drop_quantity_+%": 0,
    "map_item_drop_rarity_+%": 0,
    "map_pack_size_+%": 7,
    "map_map_item_drop_chance_+%": 10,
    "map_number_of_rare_packs_+%": 0,
    "map_number_of_magic_packs_+%": 0
  }
};
